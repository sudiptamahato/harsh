...
window.showItemDetail = function (collectionName, id, fromContext = null) {
    ...
    typeBadge.textContent = collectionName === 'adobeSoftware' ? 'Software' :
        collectionName === 'plugins' ? 'Plugin' :
            collectionName === 'scripts' ? 'Script/Extension' :
                collectionName === 'assets' ? 'Asset Pack' : 'Resource';
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.goBackToMain = function (sectionId) {
        ...
        transitionToView(mainView, () => {
            setTimeout(() => {
                // If sectionId is "plugins", default to "plugins-container" if element not found direct
                let element = document.getElementById(sectionId);
                if (!element) element = document.getElementById(sectionId + '-container');
                if (!element && sectionId === 'software') element = document.getElementById('software-container');
...
function applyWinterTheme() {
    ...
    if (typeof db === 'undefined' || !db) {
        console.warn("Firebase DB not initialized, skipping winter theme.");
        return;
    }

    db.ref('settings/winterTheme').once('value').then(snapshot => {
        const theme = snapshot.val();

        // If theme is not enabled or doesn't exist, remove effects (cleanup)
        if (!theme || !theme.enabled) {
            document.body.classList.remove('theme-winter-hat', 'theme-winter-lights', 'theme-winter-cards');
            const container = document.getElementById('snow-container');
            if (container) container.remove();
            return;
        }

        console.log("Winter Theme Enabled:", theme);

        // 1. Inject CSS if not present
        if (!document.querySelector('link[href="snow/snow.css"]')) {
            const link = document.createElement('link');
            link.rel = 'stylesheet';
            link.href = 'snow/snow.css';
            document.head.appendChild(link);
        }

        // 2. Apply Custom Classes
        if (theme.santaHat) document.body.classList.add('theme-winter-hat');
        else document.body.classList.remove('theme-winter-hat');

        if (theme.lights) document.body.classList.add('theme-winter-lights');
        else document.body.classList.remove('theme-winter-lights');

        if (theme.snowOnCards) document.body.classList.add('theme-winter-cards');
        else document.body.classList.remove('theme-winter-cards');

        // 3. Inject JS and Init Snow if not present
        if (!document.querySelector('script[src="snow/snow.js"]')) {
            const script = document.createElement('script');
            script.src = 'snow/snow.js';
            script.onload = () => {
                // Initialize snow with intensity
                if (typeof createSnowEffect === 'function') {
                    createSnowEffect(theme.intensity || 50);
                }
            };
            document.body.appendChild(script);
        } else {
            // Script already loaded, just update snow
            if (typeof createSnowEffect === 'function') {
                createSnowEffect(theme.intensity || 50);
            }
        }
    }).catch(err => console.error("Error loading winter theme:", err));
}